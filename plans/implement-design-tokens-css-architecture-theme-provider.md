...
| **TASK-012** | Create a lightweight MSBuild-integrated CSS bundler that produces a single `cymrublazor.css`, removes development-only directives (`@import` and duplicate `@layer` declarations), preserves comments and source order, and requires no external tooling. Automatically runs during both `Build` and `Publish`. |           |      |
